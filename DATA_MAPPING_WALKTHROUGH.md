...
- [ ] **Bookings**: Go to `/Bookings`. See list. Click "Details" or "Edit" on an item. Verify changes persist.
- [ ] **Payments**: Go to `/Payments`. Verify transaction list matches Booking data (if payments were created).
- [ ] **Dashboard**: Go to `/Dashboard` (or Home). Verify the stats reflect the data in other modules.
